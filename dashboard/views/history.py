"""Historical analysis page - restructured with 3 tabs: Resultats, Tendances, Analyse."""
...
def render():
    ...
    c2.metric("Vendus", f"{sold_count:,} ({sold_count / total * 100:.0f}%)" if total > 0 else "0")

    if not sold_df.empty and sold_df["final_price"].notna().any():
        c3.metric("Prix final moyen", f"{sold_df['final_price'].mean():,.0f} EUR")
    else:
        c3.metric("Prix final moyen", "N/A")

    if not sold_df.empty and sold_df["ratio"].notna().any():
        c4.metric("Ratio median", f"{sold_df['ratio'].median():.2f}x")
    else:
    ...
    # ── Tabs (3 instead of 5) ────────────────────────────────────────
    tab_results, tab_trends, tab_analysis = st.tabs([
        "Resultats",
        "Tendances",
        "Analyse",
    ])
    ...
    # ================================================================
    # TAB 1 : Resultats (data table)
    # ================================================================
    with tab_results:
        _render_results_tab(df, sold_df, total)

    # ================================================================
    # TAB 2 : Tendances (merged Visualisations + Tendances temporelles)
    # ================================================================
    with tab_trends:
        _render_trends_tab(df, sold_df)

    # ================================================================
    # TAB 3 : Analyse (merged Ratios + Geographic)
    # ================================================================
    with tab_analysis:
        _render_analysis_tab(df, sold_df, metrics, filters)


# ── Tab render functions ─────────────────────────────────────────────

def _render_results_tab(df: pd.DataFrame, sold_df: pd.DataFrame, total: int):
    """Tab 1: Data table with sortable columns."""
    ...
    display_cols = [
        ...
        "ratio", "label_status", "result_date", "tribunal_name", "licitor_url",
    ]
    ...
    col_names = {
        ...
        "property_type": "Type", "surface_m2": "Surface (m2)",
        "mise_a_prix": "Mise a prix", "final_price": "Prix final",
        "prix_m2": "Prix/m2", "ratio": "Ratio", "label_status": "Resultat",
        "result_date": "Date", "tribunal_name": "Tribunal", "licitor_url": "Lien Licitor",
    }
    ...
    st.dataframe(
        ...
        column_config={
            "Surface (m2)": st.column_config.NumberColumn("Surface (m2)", format="%.1f m2"),
            "Mise a prix": st.column_config.NumberColumn("Mise a prix", format="euro"),
            "Prix final": st.column_config.NumberColumn("Prix final", format="euro"),
            "Prix/m2": st.column_config.NumberColumn("Prix/m2", format="%.0f EUR/m2"),
            "Ratio": st.column_config.NumberColumn("Ratio", format="%.2f x"),
            "Lien Licitor": st.column_config.LinkColumn("Lien Licitor", display_text="Voir"),
        },
    )
    st.caption(f"{len(df_show)} resultats affiches sur {total} au total")


def _render_trends_tab(df: pd.DataFrame, sold_df: pd.DataFrame):
    """Tab 2: Merged visualizations + temporal trends."""
    st.subheader("Visualisations et tendances")

    # ── Row 1: Donut + Bar by department ─────────────────────────────
    col_pie, col_dept = st.columns(2)
    ...
    with col_dept:
        if df["department_code"].notna().any():
            ...
            fig_dept.add_trace(go.Bar(
                x=dept_counts["department_code"], y=dept_counts["vendus"],
                name="Vendus", marker_color="#10b981",
            ))
            fig_dept.add_trace(go.Bar(
                x=dept_counts["department_code"],
                y=dept_counts["total"] - dept_counts["vendus"],
                name="Non vendus", marker_color="#ef4444",
            ))
            apply_theme(fig_dept)
            fig_dept.update_layout(
                barmode="stack",
                title="Resultats par departement (top 25)",
                xaxis_title="Departement", yaxis_title="Nombre",
                height=400,
            )
    ...
    if not df_with_price.empty:
        max_hist = st.slider(
            ...
            step=50_000, key="hist_price_max",
        )
        df_hist = df_with_price[df_with_price["final_price"] <= max_hist]
        fig_hist = px.histogram(
            df_hist, x="final_price", nbins=40,
    ...
    if not df_dated.empty:
        ...
        fig_vol.add_trace(go.Bar(
            ...
            name="Carences", marker_color="#ef4444",
        ))
        fig_vol.add_trace(go.Bar(
            x=monthly["result_month"], y=monthly["nb_non_req"],
            name="Non requises", marker_color="#94a3b8",
        ))
        apply_theme(fig_vol)
        ...
        # Price + Ratio evolution (2 columns)
        col_price, col_ratio = st.columns(2)
        ...
        with col_price:
            ...
            if not monthly_with_price.empty:
                ...
                fig_price.add_trace(go.Scatter(
                    ...
                    mode="lines+markers", name="Prix final moyen",
                    line=dict(color="#f59e0b", width=2),
                ))
                fig_price.add_trace(go.Scatter(
                    x=monthly_with_price["result_month"],
                    y=monthly_with_price["avg_map"],
                    mode="lines+markers", name="Mise a prix moyenne",
                    line=dict(color="#06b6d4", width=2, dash="dash"),
                ))
                apply_theme(fig_price)
                fig_price.update_layout(
                    title="Evolution des prix moyens",
                    xaxis_title="Mois", yaxis_title="EUR",
                    height=400,
                )
        ...
        with col_ratio:
            ...
            if not monthly_with_ratio.empty:
                ...
                fig_ratio_t.add_trace(go.Scatter(
                    ...
                    mode="lines+markers", name="Ratio median",
                    line=dict(color="#8b5cf6", width=2),
                    fill="tozeroy",
                    fillcolor="rgba(139, 92, 246, 0.1)",
                ))
                fig_ratio_t.add_hline(y=1.0, line_dash="dash", line_color="#94a3b8",
                                      annotation_text="1x")
                apply_theme(fig_ratio_t)
                fig_ratio_t.update_layout(
                    title="Evolution du ratio median",
                    xaxis_title="Mois", yaxis_title="Ratio",
                    height=400,
                )
        ...
        fig_rate.add_trace(go.Scatter(
            x=monthly["result_month"], y=monthly["taux_vente"],
            mode="lines+markers", name="Taux de vente (%)",
        ...
        fig_rate.update_layout(
            ...
            xaxis_title="Mois", yaxis_title="% vendus",
            yaxis=dict(range=[0, 105]),
        )
        st.plotly_chart(fig_rate, use_container_width=True)


def _render_analysis_tab(df: pd.DataFrame, sold_df: pd.DataFrame, metrics: AuctionMetrics, filters: dict):
    """Tab 3: Merged ratio analysis + geographic analysis."""
    ...
    if df_ratio.empty:
        ...
        fig_scatter.update_layout(legend_title="Departement", height=550)
        ...
        with col_top:
            ...
            st.dataframe(
                df_ratio.nlargest(10, "ratio")[
                    ...
                    "final_price": "Prix final", "ratio": "Ratio", "result_date": "Date",
                }),
                use_container_width=True, hide_index=True,
            )

        with col_bottom:
            st.markdown("**Meilleurs prix (ratio le plus bas)**")
            st.dataframe(
    ...
    # Folium map (absorbed from map_view.py)
    geo_df = metrics.geographic_data()
    if not geo_df.empty:
        # Apply current filters to map data
        if filters.get("department_codes"):
            geo_df = geo_df[geo_df["department_code"].isin(filters["department_codes"])]
        if filters.get("min_price") is not None:
            geo_df = geo_df[geo_df["mise_a_prix"].fillna(0) >= filters["min_price"]]
        if filters.get("max_price") is not None:
            geo_df = geo_df[geo_df["mise_a_prix"].fillna(0) <= filters["max_price"]]

        if not geo_df.empty:
            st.markdown(f"**{len(geo_df)}** biens avec coordonnees GPS")
    ...
    if not df_dept.empty:
        ...
        # Scatter: taux de vente vs ratio
        ...
        if not dept_scatter.empty:
            fig_dep_s = px.scatter(
                dept_scatter,
                x="taux_vente", y="median_ratio",
                size="total", text="department_code",
            ...
            apply_theme(fig_dep_s)
            fig_dep_s.update_layout(height=500)
            st.plotly_chart(fig_dep_s, use_container_width=True)

        # Department table
        dept_display = dept_stats.copy()
        dept_display["avg_map"] = dept_display["avg_map"].apply(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
        dept_display["avg_final"] = dept_display["avg_final"].apply(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
        dept_display["median_ratio"] = dept_display["median_ratio"].apply(lambda x: f"{x:.2f}x" if pd.notna(x) else "")
        dept_display["taux_vente"] = dept_display["taux_vente"].apply(lambda x: f"{x:.0f}%" if pd.notna(x) else "")

        dept_display = dept_display.rename(columns={
            "department_code": "Dept", "total": "Total", "vendus": "Vendus",
            "carences": "Carences", "avg_map": "MAP moy.",
            "avg_final": "Prix final moy.", "median_ratio": "Ratio median",
        ...
        if df["tribunal_name"].notna().any():
            st.markdown("**Resultats par tribunal**")
            trib_stats = (
            ...
            trib_display["avg_final"] = trib_display["avg_final"].apply(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
            trib_display["median_ratio"] = trib_display["median_ratio"].apply(lambda x: f"{x:.2f}x" if pd.notna(x) else "")
            trib_display["taux_vente"] = trib_display["taux_vente"].apply(lambda x: f"{x:.0f}%" if pd.notna(x) else "")
            trib_display = trib_display.rename(columns={
                "tribunal_name": "Tribunal", "total": "Total", "vendus": "Vendus",
                "avg_final": "Prix final moy.", "median_ratio": "Ratio median",
                "taux_vente": "Taux vente",
            })
